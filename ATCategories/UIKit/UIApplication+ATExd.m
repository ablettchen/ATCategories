...
#import "NSArray+ATExd.h"
#import "NSObject+ATExd.h"
#import "ATCategoriesMacro.h"
#import "UIDevice+ATExd.h"
#import <sys/sysctl.h>
#import <mach/mach.h>
#import <objc/runtime.h>

ATSYNTH_DUMMY_CLASS(UIApplication_ATExd)

#define kNetworkIndicatorDelay (1/30.0)
@interface _ATUIApplicationNetworkIndicatorInfo : NSObject
@property (nonatomic, assign) NSInteger count;
@property (nonatomic, strong) NSTimer *timer;
@end

@implementation _ATUIApplicationNetworkIndicatorInfo
@end

@implementation UIApplication (ATExd)
...
- (BOOL)isPirated {
    ...
    if (![self _at_fileExistInMainBundle:@"_CodeSignature"]) {
        return YES;
    }
    
    if (![self _at_fileExistInMainBundle:@"SC_Info"]) {
        return YES;
    }
...
- (BOOL)_at_fileExistInMainBundle:(NSString *)name {
...
ATSYNTH_DYNAMIC_PROPERTY_OBJECT(networkActivityInfo, setNetworkActivityInfo, RETAIN_NONATOMIC, _ATUIApplicationNetworkIndicatorInfo *);
...
- (void)_changeNetworkActivityCount:(NSInteger)delta {
    @synchronized(self){
        dispatch_async_on_main_queue(^{
            _ATUIApplicationNetworkIndicatorInfo *info = [self networkActivityInfo];
            if (!info) {
                info = [_ATUIApplicationNetworkIndicatorInfo new];
