...
#define IS_IPHONE_X (IS_IPHONE && (SCREEN_MAX_LENGTH == 812.0 || SCREEN_MAX_LENGTH == 1344.0 || SCREEN_MAX_LENGTH == 896.0 || SCREEN_MAX_LENGTH == 1218.0))
 ...
 *******************************************************************************
 Example:
 ATSYNTH_DUMMY_CLASS(NSString_ATExd)
 */
...
#define ATMUTEX_ASSERT_ON_ERROR(x_) do { \
__unused volatile int res = (x_); \
assert(res == 0); \
} while (0)
    assert(mutex != NULL);
    if (!recursive) {
        ATMUTEX_ASSERT_ON_ERROR(pthread_mutex_init(mutex, NULL));
    } else {
        pthread_mutexattr_t attr;
        ATMUTEX_ASSERT_ON_ERROR(pthread_mutexattr_init (&attr));
        ATMUTEX_ASSERT_ON_ERROR(pthread_mutexattr_settype (&attr, PTHREAD_MUTEX_RECURSIVE));
        ATMUTEX_ASSERT_ON_ERROR(pthread_mutex_init (mutex, &attr));
        ATMUTEX_ASSERT_ON_ERROR(pthread_mutexattr_destroy (&attr));
    }
#undef ATMUTEX_ASSERT_ON_ERROR
}


#endif /* ATCategoriesMacro_h */
